...
export function ReportPreview({
  ...
  const report = useMemo(
    () => (parcel ? buildParcelReport(parcel, tasks) : null),
    [parcel, tasks],
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center overflow-y-auto bg-foreground/40 px-4 py-8">
      <div className="relative w-full max-w-3xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border border-border bg-paper px-5 py-3">
          <span className="rule-label">Report preview — {p.address}</span>
          <button
            onClick={onClose}
            className="border border-border px-3 py-1 font-mono text-xs text-muted-foreground hover:border-primary hover:text-primary"
          >
            Close
          </button>
        </div>
        ...
        <div className="border border-t-0 border-border bg-paper">
          ...
          <div className="px-8 py-10 sm:px-12">
            ...
            <h2 className="mt-3 font-serif text-3xl leading-tight text-primary">{p.address}</h2>
            ...
            <ReportSection title="1. Parcel summary">
              <dl className="divide-y divide-border border-y border-border">
                ...
                <Row label="Acreage" value={`${p.acreage.toFixed(2)} acres`} citeId="assessor-record" />
                <Row label="Zoning" value={z.label} citeId={zc.district} />
                <Row label="Assessed value" value={currency.format(p.assessedValue)} mono citeId="assessor-value" />
            ...
            <ReportSection title="3. Funding sources by how reachable they are">
              <p className="text-sm leading-relaxed text-muted-foreground">
                Eligibility says the parcel qualifies. These tiers say whether your organization can
                realistically get the money, in what timeframe, and at what effort. Nothing has been
                removed — hard-to-reach sources sit lower with the reason attached.
              </p>
              <LevelFilterControl value={levelFilter} onChange={setLevelFilter} className="mb-4 mt-4" />
                          ...
                          <div className="grid grid-cols-[minmax(0,1fr)_13rem] gap-4">
                            <div>
                              <div className="flex items-start justify-between gap-4">
                                ...
                                <span
                                  className={`shrink-0 border px-2 py-0.5 text-[11px] leading-tight ${statusClass(prog.status)}`}
                                >
                                  {prog.status}
                                </span>
                              </div>
            ...
            >
              ...
              <ReportSection title="6. Underwriting limits">
                <p className="text-sm leading-relaxed text-muted-foreground">
                  ...
                  parcel's metropolitan area — with the binding limit called out when programs are
                  layered.
                </p>
              ...
              <ReportSection title="7. Cash flow summary">
                <p className="text-sm leading-relaxed text-muted-foreground">
                  Eligibility says whether a source is open to you. This says when its money is in
                  your account.
                </p>
              ...
              <ReportSection title="10. Key dates and deadlines">
                <dl className="divide-y divide-border border-y border-border">
                  {report.dates.map((d) => (
                    <div key={d.label} className="grid grid-cols-[7rem_minmax(0,1fr)] gap-3 py-3">
...
function Paywall() {
  return (
    <div className="absolute inset-x-0 bottom-0 top-16 flex items-start justify-center px-6 pb-10">
      <div className="w-full max-w-2xl border-2 border-primary bg-paper p-6 sm:p-8">
        <p className="rule-label">Continue reading</p>
        <h3 className="mt-2 font-serif text-2xl leading-snug text-primary">
          Action plans for the remaining programs, the stacking analysis, and the saved parcel dashboard.
        </h3>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-[1.15fr_1fr]">
          <div className="border-2 border-primary bg-secondary p-5">
            <div className="flex items-center justify-between">
              <span className="rule-label">Monthly subscription</span>
              <span className="bg-accent px-2 py-0.5 text-[11px] text-primary-foreground">
                Recommended
              </span>
            </div>
            <p className="mt-3 font-mono text-lg text-primary">[PRICE PLACEHOLDER] / month</p>
            <ul className="mt-3 space-y-1.5 text-sm text-foreground">
              <li className="flex gap-2">
                <span className="text-primary">▪</span> Action plans for every eligible program
              </li>
              <li className="flex gap-2">
                <span className="text-primary">▪</span> Stacking analysis and saved parcel dashboard
              </li>
              <li className="flex gap-2">
                <span className="text-primary">▪</span> Unlimited chat questions
              </li>
            </ul>
            <button className="mt-5 w-full border border-primary bg-primary px-4 py-3 text-sm font-medium tracking-wide text-primary-foreground transition-colors hover:bg-primary-deep">
              Start subscription
            </button>
          </div>
          ...
          <div className="border border-border bg-paper p-5">
            ...
            <p className="mt-3 font-mono text-lg text-foreground">[PRICE PLACEHOLDER] once</p>
            ...
            <button className="mt-5 w-full border border-primary px-4 py-3 text-sm font-medium tracking-wide text-primary transition-colors hover:bg-secondary">
              Buy this report
            </button>
          </div>
        </div>
...
function ReportSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="mt-10">
      <h3 className="rule-label border-b border-primary pb-2">{title}</h3>
...
function Row({
  ...
  return (
    <div className="grid grid-cols-[8rem_minmax(0,1fr)] gap-3 py-2.5">
      ...
      <dd className={`text-sm leading-relaxed text-foreground ${mono ? "font-mono" : ""}`}>
        {value}
